...
public:
    ...
    bool another_inorder(Node *n,int height)
    {
        int h;
         if(n==NULL)
        {
        ...
        cout << n->data << "\t";
        another_inorder(n->right,height);
    }
    ...
    Node *leave_nodes_level(Node *curr,int maxHeight)
    {
        // int max_height = maxDepth(Root);
        cout<<"Max height is "<<maxHeight<<endl;
        bool yes=another_inorder(curr,maxHeight);
        if(yes==true)
        {

            cout<<"All leaves nodes are on the same level"<<endl;
        }   
        else 
        {
            cout<<"All leaves nodes are not on the same level"<<endl;
        }
    }
};